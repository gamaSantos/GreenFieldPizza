...
class CustomerEntity
{
    ...
    {
        var adress = new Adress(StreetName, StreetNumber, AdressObservation);
        var phone = new Telephone(PhoneNumber, PhoneRegion);
        var name = new Name(FirstName, LastName);
        var loadResult = Customer.Load(Id, name, phone, adress);
        if (loadResult.TryGetContent(out var customer) == false) throw new InvalidDataException(string.Join(',', loadResult.Errors));
        return customer;
    }
